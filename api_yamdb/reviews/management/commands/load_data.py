...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open('static/data/users.csv', encoding='utf8') as input_file:
            input_file_without_header = input_file.readlines()[1:]
            for row in input_file_without_header:
                id, username, email, role, bio, first_name, last_name = row.split(',')
                user = User(
                    id=id,
                    username=username,
                    email=email,
        ...
        with open('static/data/review.csv', encoding='utf8') as input_file:
            ...
            for row in input_file_without_header:
                if '"' in row:
                    ...
                    id_author, score, pub_date = row[closing_quotation + 1:].split(',')
                    review = Review(
                        id=id,
        ...
        with open('static/data/comments.csv', encoding='utf8') as input_file:
            ...
            for row in input_file_without_header:
                if '"' in row:
                    ...
                    text = row[opening_quotation + 1: closing_quotation - 1]
                    id_author, pub_date = row[closing_quotation + 1:].split(',')
                    comment = Comment(
                        id=id,
